...
- `@amit-t/stet-md` npm package metadata with `stet-md` and `stmd` binaries.
- `stet-md` binary and `stmd` alias.
- `stet-md FILE.md` local loopback review server.
- Browser UI for heading, paragraph, and document-level comments.
- Thread reply, resolve, and reopen flows.
- `stet-md list --json`, `reply`, `resolve`, `reopen`, and MVP `comment` helper.
