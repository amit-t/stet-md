...
Redline stores review threads inside Markdown files. The Markdown file is the source of truth.
...
redline reply FILE.md --thread THREAD_ID --author Claude --message "I changed the section above."
...
1. Do not delete `redline:thread` blocks unless Amit explicitly asks.
...
Message bodies containing `--` or `-->` must be encoded. The CLI does this automatically using `body_base64:`.
