...
describe("parseThreads", () => {
  ...
  it("parses one structured thread after a heading", () => {
    const src = `## H\n\n${renderThreadBlock(thread("stt_20260607_150015_aaaaaa", "hi"))}\n`;
    const threads = listThreads(src);
    expect(threads).toHaveLength(1);
    expect(threads[0]!.id).toBe("stt_20260607_150015_aaaaaa");
    expect(threads[0]!.messages[0]!.bodyMarkdown).toBe("hi");
  });

  it("parses multiple threads in one section in document order", () => {
    const a = renderThreadBlock(thread("stt_20260607_150015_aaaaaa", "first"));
    const b = renderThreadBlock(thread("stt_20260607_150016_bbbbbb", "second"));
    const src = `## H\n\n${a}\n\n${b}\n`;
    const ids = listThreads(src).map((t) => t.id);
    expect(ids).toEqual([
      "stt_20260607_150015_aaaaaa",
      "stt_20260607_150016_bbbbbb",
    ]);
  });
  ...
  it("extracts messages from the structured marker, not the blockquote", () => {
    ...
    const block = renderThreadBlock(thread("stt_20260607_150015_aaaaaa", "TRUTH"));
  ...
  it("flags a divergent generated blockquote", () => {
    const block = renderThreadBlock(thread("stt_20260607_150015_aaaaaa", "real"));
  ...
  it("ignores marker tokens inside fenced code and inline code", () => {
    const real = renderThreadBlock(thread("stt_20260607_150015_aaaaaa", "real"));
    const src =
      "# Docs\n\n" +
      "Mention `<!-- stet:thread ... -->` inline in prose.\n\n" +
      "```markdown\n<!-- stet:thread\nid: stt_example\n-->\n> example\n<!-- /stet:thread -->\n```\n\n" +
      real +
      "\n";
    const { blocks, errors } = scanThreadBlocks(src);
    ...
    expect(blocks[0]!.thread!.id).toBe("stt_20260607_150015_aaaaaa");
  });

  it("preserves unknown Markdown around thread blocks (raw range is exact)", () => {
    const block = renderThreadBlock(thread("stt_20260607_150015_aaaaaa", "hi"));
    const src = `before\n\n${block}\n\nafter\n`;
