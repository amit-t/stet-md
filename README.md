...
- commentable blocks everywhere: headings, paragraphs, lists, tables, blockquotes, and fenced code are all comment targets — including blocks inside `<details>` collapsibles at any nesting depth (`<summary>` lines, `<hr>`, and the frontmatter block are not targets). CLI selectors: `paragraph:N`, `heading:N`, `list:N`, `blockquote:N`, `table:N`, `code_block:N`, `document`;
